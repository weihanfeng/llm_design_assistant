...
class Architecture_idea_exploration_agent():
  ...
  def architecture_brief_question_agent(self):
    return Agent(
        ...
        backstory=
        'An expert in understanding the requirements of the architecture design brief through a series of questions. You should only ask a maximum of 5 questions.',
        verbose=True,
        llm=self.llm,
  ...
  def concept_generation_agent(self):
    return Agent(
        ...
        goal="""Given a architecture design brief and some research, generate 5 vastly different architecture design concepts which are unique, interesting, innovative and meets the requirements of the brief.""",
